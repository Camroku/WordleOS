...
void terminal_move_cursor()
{
    ...
    uint16_t cursorLocation = term_cur_y * 80 + term_cur_x;
...
// Moves the cursor to given x and y
void terminal_move_cursor_to(int cx, int cy)
{
    term_cur_y = cx;
    term_cur_x = cy;
 ...
 > 0)
    {
        term_cur_y
    --;
        term_cur_x = line_length[term_cur_y
    ];
    }
    location = buffer + (term_cur_y
 * 80 + term_cur_x);
